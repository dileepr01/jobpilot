...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  ...
  if (profileError || !profile?.resume_text) {
    return NextResponse.json(
      { error: 'A source resume is required to create a truthful tailored resume.' },
      { status: 400 }
    )
  }
  ...
  const jobRelation = match.jobs as unknown as {
    title: string
    company: string
    description: string
  }
  ...
  kit.applicationPack = buildApplicationPack({
    jobTitle: jobRelation.title,
    company: jobRelation.company,
    jobDescription: jobRelation.description,
    kit
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      application_pack: kit.applicationPack,
      status: 'reviewed'
    })
    .eq('id', params.id)
    .eq('user_id', user.id)
